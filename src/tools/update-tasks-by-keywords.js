...
import { fuzzyMatchKeywords } from './fuzzy-matching-utils.js';
import { 
...
/**
 * Register the update-tasks-by-keywords tool
 * @param {Object} server - FastMCP server instance
 */
export function registerUpdateTasksByKeywordsTool(server) {
  server.addTool({
    name: 'update_tasks_by_keywords',
    description: 'Update tasks that match specified keywords using AI guidance with scalable batch processing. Uses fuzzy keyword matching to find relevant tasks.',
    parameters: z.object({
      projectRoot: z
        .string()
      ...
      minScore: z
        ...
        .optional()
        .default(0.3)
        .describe('Minimum fuzzy match score for keyword matching (0-1, default: 0.3)'),
      maxDepth: z
        .number()
        .optional()
        .default(2)
      ...
      includeSubtasks: z
        ...
        .optional()
        .default(false)
        .describe('Include subtasks in keyword matching and updates')
    }),
    execute: async (args) => {
      try {
        const { 
        ...
        logger.info(`Preparing keyword-based task update instructions with keywords: ${keywords.join(', ')}`);
        ...
        // Build keyword-based relevant tasks chain
        ...
        if (targetTaskIds.size === 0) {
          return createErrorResponse(`No tasks found matching keywords: ${keywords.join(', ')} with minimum score ${minScore}`);
        }
        ...
        if (tasksToUpdate.length === 0) {
          return createErrorResponse(`No updatable tasks found matching keywords. All matching tasks are already completed.`);
        }
        ...
        logger.info(`Keyword-based batch strategy: ${batchConfig.useBatches ? 'BATCHED' : 'SINGLE'}, ` +
                   `${batchConfig.totalBatches} batch(es), ${batchConfig.batchSize} tasks per batch, ` +
                   `~${batchConfig.estimatedTokens} estimated tokens`);
        
        // Generate system prompt
        const systemPrompt = `You are an AI assistant helping to update software development tasks based on new context and keyword-based filtering.
You will be given ${batchConfig.useBatches ? 'a batch of' : 'a set of'} tasks that were selected based on their keywords matching: ${keywords.join(', ')}.
Your job is to update these keyword-matched tasks to reflect the changes described in the prompt, while preserving their basic structure.
...
4. Update flowNames arrays if business flows are affected (1-4 business flow names per task)
...
IMPORTANT: Since these tasks were selected based on keyword matching, ensure the updates maintain semantic coherence with the original keywords: ${keywords.join(', ')}`;

        if (batchConfig.useBatches) {
          // Create batched update instructions
          const instructions = `I can guide you through updating keyword-matched tasks using a scalable batched approach. Here's exactly what you need to do:
...
2. **Apply the user prompt**: Here are the keyword-matched tasks to update (for current batch):
   [BATCH_TASKS_JSON]
   ...
   KEYWORD CONTEXT: These tasks were selected because their keywords matched: ${keywords.join(', ')}. Ensure updates maintain semantic coherence with these concepts.
...
Please proceed with processing Batch 1 now based on the instructions above.`;
          ...
          const result = {
            success: true,
            action: 'update_tasks_by_keywords_batched_guidance',
            ...
            parameters: {
              ...
              batchConfig,
              keywordMatchedTaskIds: Array.from(targetTaskIds)
            },
            instructions
          };
          
          logger.info(`Generated batched keyword-based task update instructions for ${tasksToUpdate.length} tasks in ${batchConfig.totalBatches} batches${research ? ' with research mode' : ''}`);
          ...
          const userPrompt = `Here are the keyword-matched tasks to update:\n${taskDataString}\n\nPlease update these tasks based on the following new context:\n${prompt}\n\nIMPORTANT: In the tasks JSON above, any subtasks with "status": "done" or "status": "completed" should be preserved exactly as is. Build your changes around these completed items.\n\nKEYWORD CONTEXT: These tasks were selected because their keywords matched: ${keywords.join(', ')}. Ensure updates maintain semantic coherence with these concepts.\n\nReturn only the updated tasks as a valid JSON array.`;
          
          const instructions = `I can guide you through updating keyword-matched tasks based on new context. Here's exactly what you need to do:
...
**YOUR NEXT ACTION:**
Please now analyze the keyword-matched tasks and new context provided above and generate the updated tasks JSON according to the specifications. Once you create the JSON response, you should:
...
Please proceed with updating the keyword-matched tasks now based on the context and instructions above.`;

          const result = {
            success: true,
            action: 'update_tasks_by_keywords_guidance',
            targetFile: tasksPath,
            parameters: {
              keywords,
              prompt,
          ...
          logger.info(`Generated keyword-based task update instructions for ${tasksToUpdate.length} tasks${research ? ' with research mode' : ''}`);
          return createContentResponse(result);
        }
        
      } catch (error) {
        logger.error(`Failed to prepare keyword-based task update instructions: ${error.message}`, { error: error.stack, args });
        return createErrorResponse(`Failed to prepare keyword-based task update instructions: ${error.message}`);
      }
    }
  });
}
